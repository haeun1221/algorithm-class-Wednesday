...
class LinearQueueNoReset:
    ...
    def is_empty(self):     # 큐가 비어있는지 여부 확인
        return self.front == self.rear

    def is_full(self):      # 큐가 가득 찼는지 여부 확인
        return self.rear == self.capacity - 1

    def size(self):         # 현재 큐에 저장된 요소의 개수
        return self.rear - self.front

    def enqueue(self, item):        # 큐에 요소 추가
        if self.is_full():
            raise Exception("Queue is full")
        self.rear += 1
        self.array[self.rear] = item

    def dequeue(self):              # 큐에서 요소 제거하고 반환
        if self.is_empty():
            raise Exception("Queue is empty")
        self.front += 1
        return self.array[self.front]

    def peek(self):                 # 큐의 맨 앞 요소 반환 (제거x)
        if self.is_empty():
            raise Exception("Queue is empty")
        return self.array[self.front + 1]

    def display(self, msg="LinearQueueNoReset"):    # 큐의 상태 출력
        print(f"{msg}: front={self.front}, rear={self.rear}, "
              f"size={self.size()}/{self.capacity}", end=" | ")

        # 전체 슬롯(0..capacity-1)을 순서대로 출력
        # active 구간(front < i <= rear)은 실제 값, 그 외는 None으로 표시
        print("[", end="")
